...
cifar_C = cifar_C.reshape(cifar_image_array_train.shape[0], 28 ** 2)

cifar_Cd = np.hstack((cifar_A, cifar_B, cifar_C))
...
cifar_model.add(Dense(Nh, input_dim=3 * 28 ** 2, activation='linear', use_bias=False))
...
Ct_PennFudanPed = Ct_PennFudanPed.reshape(image_array_test.shape[0], 28 ** 2)

Cdt_PennFudanPed = np.hstack((At_PennFudanPed, Bt_PennFudanPed, Ct_PennFudanPed))
