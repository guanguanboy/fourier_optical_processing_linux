...
Nh = 1400 # 隐藏层节点数目
model = Sequential()
model.add(Dense(Nh, input_dim=3*28**2, activation = 'linear', use_bias=False)) # Nh 是这个全连接层输出节点的个数
#model.add(Dropout(0.06))
model.add(Dense(28**2, input_dim=Nh, activation = 'relu', use_bias=True)) # 28**2 是这个全连接层输出节点的个数
