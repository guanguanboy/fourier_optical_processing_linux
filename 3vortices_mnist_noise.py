...
#通过加载文件夹中的图片来够着x_train, x_test
# 读取D:\DataSets\mnist_dataset_noise\train文件夹下的数据，并使用已训练好的网络进行处理
# 读取图片作为输入训练网络
noise_mnist_image_root_dir = 'D:/DataSets/mnist_dataset_noise/train'
# image_path = os.path.join(image_root_dir, self.type_name)
noise_mnist_img_name_list = os.listdir(noise_mnist_image_root_dir)  # 得到path目录下所有图片名称的一个list

noise_mnist_image_list = []

image_count = len(noise_mnist_img_name_list)
print('image_count:')
print(image_count)
for i in range(image_count):
    img_item_path = os.path.join(noise_mnist_image_root_dir, noise_mnist_img_name_list[i])
    img = io.imread(img_item_path)
    noise_mnist_image_list.append(img)

# 将list转换为ndarray
noise_mnist_image_array_train = np.array(noise_mnist_image_list)
print(noise_mnist_image_array_train.shape)
print(type(noise_mnist_image_array_train))

x_train = noise_mnist_image_array_train
x_num = x_train.shape[0]

# 读取D:\DataSets\mnist_dataset\test文件夹下的数据，并使用已训练好的网络进行推理
# 进行predict，然后计算loss等
image_root_dir = 'D:/DataSets/mnist_dataset/test'
# image_path = os.path.join(image_root_dir, self.type_name)
...
C = C.reshape(x_train.shape[0], 28 ** 2)
...
Ct = Ct.reshape(x_test.shape[0], 28 ** 2)

Cdt = np.hstack((At, Bt, Ct))
Cdt_show = Cdt.reshape(x_test.shape[0], 28, 84)
...
Nh = 1400
model = Sequential()
model.add(Dense(Nh, input_dim=3*28**2, activation = 'linear', use_bias=False))
